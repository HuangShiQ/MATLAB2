...
for d = data(1).TEST_indices'
 [data(d).TEST_dia_endo_total_area, ~, ~] = trifacet_area3D(data(d).diastolic.endo.tri, data(d).diastolic.endo.xyz);    
   [data(d).TEST_dia_epi_total_area, ~, ~] = trifacet_area3D(data(d).diastolic.epi.tri, data(d).diastolic.epi.xyz);
    
   [data(d).TEST_sys_endo_total_area, ~,~] = trifacet_area3D(data(d).systolic.endo.tri, data(d).systolic.endo.xyz);
   [data(d).TEST_sys_epi_total_area, ~, ~] = trifacet_area3D(data(d).systolic.epi.tri, data(d).systolic.epi.xyz);

   [data(d).TEST_dia_myo_total_area, ~, ~] = trifacet_area3D(data(d).diastolic.myo.tri, data(d).diastolic.myo.xyz);
   [data(d).TEST_sys_myo_total_area, ~, ~] = trifacet_area3D(data(d).systolic.myo.tri, data(d).systolic.myo.xyz);
end
