...
load('C:\Users\jesu2687\Documents\MATLAB\ONBI_short_project_1\CopyofFirstAuthorName.mat');
data(1).TEST_indices = CopyofFirstAuthorName(2:end,1)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
plot(specificity(find(accuracy == max(accuracy))), sensitivity(find(accuracy == max(accuracy))),'r*','markersize',20)
legend ('sensitivity versus specificity', 'maximum accuracy','location','best')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% CALCULATE MESH AREAS (includes SPHERICITY calculations)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
TEST_sys_endo_AVratios = cell2mat({data(:).TEST_sys_endo_AVratio})' ;
TEST_dia_endo_AVratios = cell2mat({data(:).TEST_dia_endo_AVratio})' ;
TEST_sys_epi_AVratios = cell2mat({data(:).TEST_sys_epi_AVratio})' ;
TEST_dia_epi_AVratios = cell2mat({data(:).TEST_dia_epi_AVratio})' ;
...
initial_reference_id = data(1).DETERMINE_indices(1); %sets which case will be the initial reference
TEST_initial_reference_id = data(1).TEST_indices(1); 
...
TEST_dia_endo_reference = data(TEST_initial_reference_id).diastolic.endo.xyz(1:1089,:);
TEST_dia_epi_reference = data(TEST_initial_reference_id).diastolic.epi.xyz(1:1089,:);
TEST_sys_endo_reference = data(TEST_initial_reference_id).systolic.endo.xyz(1:1089,:);
TEST_sys_epi_reference = data(TEST_initial_reference_id).systolic.epi.xyz(1:1089,:);
...
TEST_dia_myo_reference = reshape([TEST_dia_endo_reference(:) ; TEST_dia_epi_reference(:)], [2*1089 3]);
TEST_sys_myo_reference = reshape([TEST_sys_endo_reference(:)  ; TEST_sys_epi_reference(:)], [2*1089 3]);

dia_sys_myo_reference = [dia_myo_reference ; sys_myo_reference ];
TEST_dia_sys_myo_reference = [TEST_dia_myo_reference ; TEST_sys_myo_reference ];
...
concatIndices = [data(1).DETERMINE_indices  ; data(1).MESA_indices]';
TESTindices = data(1).TEST_indices';
%p = number of times procrustes is performed.
procrustes_iterations = 3;
[data, dia_myo_mean, sys_myo_mean, dia_sys_myo_mean, all_training_diastolic_myo_shapes, all_training_systolic_myo_shapes , all_training_dia_sys_myo_shapes] = calcProcrustes(data, procrustes_iterations, dia_myo_reference, sys_myo_reference, dia_sys_myo_reference,concatIndices);
[data, TEST_dia_myo_mean, TEST_sys_myo_mean, TEST_dia_sys_myo_mean, TEST_training_diastolic_myo_shapes, TEST_training_systolic_myo_shapes , TEST_training_dia_sys_myo_shapes] = calcProcrustes(data, procrustes_iterations, TEST_dia_myo_reference, TEST_sys_myo_reference, TEST_dia_sys_myo_reference, TESTindices);
...
for m = sort(data(1).TEST_indices')
    TEST_diastolic_myo_shapes(m,:) = TEST_training_diastolic_myo_shapes(m,:);
    TEST_systolic_myo_shapes(m,:) = TEST_training_systolic_myo_shapes(m,:);
    
    TEST_dia_sys_myo_shapes(m,:) = TEST_training_dia_sys_myo_shapes(m,:);
...
TEST_diastolic_myo_shapes( ~any(TEST_diastolic_myo_shapes,2), : ) = [];
TEST_systolic_myo_shapes( ~any(TEST_systolic_myo_shapes,2), : ) = [];
TEST_dia_sys_myo_shapes( ~any(TEST_dia_sys_myo_shapes,2), : ) = [];
...
figure
for T = 1:100  
    title 'TEST, all four shapes'
    plot3D( reshape(TEST_dia_sys_myo_shapes(T,:),[4*1089 3]) ,'o')
    hold on
end
...
%% Find standard deviation of shape variations (model parameter values (b) , using +/- sqrt(eigenvalue) as b)
    ...
    for i = data(1).MESA_indices'
     ...
     for i = data(1).TEST_indices'
        for d = find(data(1).TEST_indices==i)
            %         d
            TEST_dia_myo_b(d,b) = (principle_dia_myo_eigenvectors(:,b)')*(data(i).diastolic.myo.xyz(:) - mean_diastolic_myo_shape');
            TEST_sys_myo_b(d,b) = (principle_sys_myo_eigenvectors(:,b)')*(data(i).systolic.myo.xyz(:) - mean_systolic_myo_shape');
            TEST_dia_sys_myo_b(d,b) = (principle_dia_sys_myo_eigenvectors(:,b)')*(data(i).dia_sys.myo.xyz(:) - mean_dia_sys_myo_shape');
...
% ERNESTO PCA b values for all 100 modes
...
DATA = [DETERMINE_dia_sys_dEPI2ENDOs ; MESA_dia_sys_dEPI2ENDOs]';
m = mean(MESA_dia_sys_dEPI2ENDOs',2);
RES = bsxfun(@minus,DATA,m);
[u,s,v] = svd(RES, 'econ');
diag(s)
Bcoefs = u.' * RES;
max( abs( m + u*Bcoefs(:,2) - DATA(:,2) ));
PCA_bvalues_all100 = [Bcoefs(1:100,1:100)' ; Bcoefs(1:100,101:200)'];
%
...
%% PCA analysis of 'ratio thickness' (Ernesto)
for i = [data(1).DETERMINE_indices ; data(1).MESA_indices]'
ratios(i,:) = data(i).dia_dEPI2ENDO ./ data(i).sys_dEPI2ENDO;
end
ratios( all(~ratios,2), : ) = [];
ratio_thickness = ratios;
...
paramIndices = [1:size(trainingdata,2)];
...
%"The OOBPermutedVarDeltaError array stores the increase in MSE averaged
%over all trees in the ensemble and divided by the standard deviation taken over the trees, for each variable"
%http://uk.mathworks.com/help/stats/ensemble-methods.html?searchHighlight=OOBPermutedVarDeltaError
...
importances(:,1) = [1:size(BaggedEnsemble.OOBPermutedVarDeltaError,2)]';
importances(:,2) = BaggedEnsemble.OOBPermutedVarDeltaError';
%%
figure
hold on
% subplot 121
...
BaggedEnsemble.OOBPermutedVarDeltaError=thickness_mode_importances
BaggedEnsemble.OOBPermutedVarDeltaError=all_72_feature_importances
  bar(all_72_feature_importances);
% bar(BaggedEnsemble.OOBPermutedVarDeltaError,0.6,'FaceColor',[0.6 0.9 1]);
plot(BaggedEnsemble.OOBPermutedVarDeltaError,'-','linewidth',2)
...
% % important3 = trainingdataLabels(find(BaggedEnsemble.OOBPermutedVarDeltaError == sorted_importance(3)),:)
 set(gca,'XTick',[0:10:size(BaggedEnsemble.OOBPermutedVarDeltaError,2)])
%  set(gca,'XTickLabel',trainingdataLabels(paramIndices(1:1:end),:))

set(gca,'XTickLabel',0:10:size(BaggedEnsemble.OOBPermutedVarDeltaError,2))
ylim ([ -0.3 1.5])
xlim ([ 0 72])
fontsize = 18;
set(gca,'XTickLabelRotation',0,'TickLength',[0.01 0.6],'FontSize',fontsize)
grid on
grid minor
set(gca, 'GridAlpha', 0.1)
important_features = [1;14;30;50;54;64]
markersize = 5;

plot(2:16,BaggedEnsemble.OOBPermutedVarDeltaError(2:16),'o','markersize',markersize,'color','y','MarkerFaceColor','y') %shape modes
plot(17:20,BaggedEnsemble.OOBPermutedVarDeltaError(17:20),'o','markersize',markersize,'color','m','MarkerFaceColor','m') %sphericity
plot(21:31,BaggedEnsemble.OOBPermutedVarDeltaError(21:31),'o','markersize',markersize,'color','c','MarkerFaceColor','c') %thickness stats
plot(32:40,BaggedEnsemble.OOBPermutedVarDeltaError(32:40),'o','markersize',markersize,'color','r','MarkerFaceColor','r') %segment thickness variance
plot(41:49,BaggedEnsemble.OOBPermutedVarDeltaError(41:49),'o','markersize',markersize,'color','g','MarkerFaceColor','g') %segment thickness mean
plot(50:53,BaggedEnsemble.OOBPermutedVarDeltaError(50:53),'o','markersize',markersize,'color','b','MarkerFaceColor','b') %Volumes
plot(54:57,BaggedEnsemble.OOBPermutedVarDeltaError(54:57),'^','markersize',markersize,'color','b','MarkerFaceColor','b') %log volumes
plot(58:72,BaggedEnsemble.OOBPermutedVarDeltaError(58:72),'o','markersize',markersize,'color','k','MarkerFaceColor','k') %Thickness modes
plot(important_features,BaggedEnsemble.OOBPermutedVarDeltaError(important_features),'k*','markersize',15) %,0.6,'FaceColor',[0.6 0.9 1]);
labels = num2str((1:size(BaggedEnsemble.OOBPermutedVarDeltaError,2))','%d');    %'
text(importances(important_features,1)+2, importances(important_features,2)+0.05,trainingdataLabels(important_features,:), 'horizontal','left', 'vertical','bottom','rotation',45,'FontSize',fontsize,'BackgroundColor','none')
...
rng(1);
k = 5
kfold_cross_validatedBE = fitensemble(trainingdata(:,50),names,'Bag',300,'Tree','type','classification','kfold',k);
kfoldlossplot = kfoldLoss(kfold_cross_validatedBE,'mode','cumulative')
plot(all_72_features__kfoldlossplot,'r-.','linewidth',2); %kfold =5

%  
%LEAVE-ONE-OUT
% when 'leaveout' is set to 'on' , kfold = number of observations 
rng(1);
leaveout_cross_validatedBE = fitensemble(trainingdata(:,50),names,'Bag',300,'Tree','type','classification','leaveout','on');
kfoldlossLOOplot = kfoldLoss(leaveout_cross_validatedBE, 'mode','cumulative')
plot(all_72_features__kfoldlossLOOplot,'k-','linewidth',2); %kfold =200
set(gca,'FontSize',18)
...
legend 'kfold, k = 5' ' leave one out' 'optimum ejection fraction error'
...
%% TEST CLASSIFIER
rng(1);
BE = fitensemble(trainingdata(:,paramIndices),names,'Bag',300,'Tree','type','classification') %
[TESTclassification.labels, TESTclassification.scores] = predict(BE, TESTdata);
%%
MESA_num = sum((TESTclassification.labels == 'm'))
DETERMINE_num = sum((TESTclassification.labels == 'd'))
figure
bar(TESTclassification.scores(:,1))
legend 'DETERMINE probability'

% finding the threshold applied by 'predict()' to the final results
min(TESTclassification.scores(find(TESTclassification.labels == 'd'),1))
max(TESTclassification.scores(find(TESTclassification.labels == 'd'),2))
min(TESTclassification.scores(find(TESTclassification.labels == 'm'),2))
max(TESTclassification.scores(find(TESTclassification.labels == 'm'),1))
